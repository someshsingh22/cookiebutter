...
for batch in trange(0, len(df), batch_size):
    ...
    outputs = llm.generate([{"prompt": prompt,"multi_modal_data": image_data_i} for image_data_i in image_data],
    ...
    os.system(f"aws s3 cp out/completed_{batch}.jsonl s3://crawldatafromgcp/somesh/emotion/captions/completed_{batch}.jsonl")

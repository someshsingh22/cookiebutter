...
def filter_all_jsonl(input_bucket, folder_path, output_bucket, substring, suffix='_filtered_chess'):
    ...
    for key in tqdm.tqdm(keys):
        ...
        if s3.list_objects_v2(Bucket=output_bucket, Prefix=new_key)['KeyCount'] > 0:
            logging.debug(f"Skipping {input_bucket}/{key} because {output_bucket}/{new_key} already exists")
            continue
        filtered_rows = filter_and_dump_jsonl(input_bucket, key, substring)
        if len(filtered_rows) == 0:
            logging.warning(f"No data containing '{substring}' found in {input_bucket}/{key}")
            continue

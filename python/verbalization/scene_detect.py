...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Filter JSONL.ZST files in an AWS S3 bucket based on a substring.')
    parser.add_argument('--process_idx', type=int, default=0,help='Index of the current process (default: 0)')
    parser.add_argument('--num_process', type=int, default=16,help='Total number of processes (default: 16)')
...
if __name__ == "__main__":
    ...
    for _suffix in tqdm(suffix):
        ...
        try:
            ...
            video_id= _suffix.split('/')[1].split('.')[0]
            save_images(scene_list=scene_list, video=video, image_name_template=f'{video_id}-$SCENE_NUMBER', output_dir='yt_scenes', num_images=1)

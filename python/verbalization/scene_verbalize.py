...
for i in trange(0, len(df), batch_size):
    try:
        ...
        inputs = processor(batch_prompts, images=images_batch, return_tensors="pt", padding=True).to(device)
        output = model.generate(**inputs, max_new_tokens=160, use_cache=True,do_sample=True,temperature=0.4,top_p=0.8)
